...
<table width="800"  align="center" border="0">
  <tr>
    <td><SECTION id=typography>
	<DIV class=page-header>
  <H1>Top Books in the shop</H1></DIV>
<p>
</p>
<?php
	//DISPLAYS A FEW MOST DOWNLOADED BOOKS
...
<?php }else { 
	...
	$tableRows = 1;
	$tableCols = 1;
	echo '<table class="table table-bordered" width="60%" height ="100"border="0"><tr>';
	for ($i=0; $i<count($books);$i++) {
		$book = $books[$i];
		echo '<td><a href="product.php?book='.$book->getId().'"><img class="img-rounded" name="'.$book->getName().'" src="images/prod_icons/'.$book->getIcon().'" width="200" height="200" alt="'.$book->getName().'" style="background-color: #339966; max-width:100; max-height:100;"></a></td>';
		if ($tableCols == 4) {
			$tableCols = 2;
			$tableRows++;
