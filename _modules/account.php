...
class Account{
    ...
    public function register($username, $email, $country, $pass1, $pass2, $spamFilter, $toc, $cellnum, $postalAddr, $fullname) {
        ...
        #   VALIDATING AND VERIFYING PASSWORD MAKING SURE THAT THE RE-ENTERED PASSWORD MATCHES WITH THE FIRST PASSWORD
        if (empty($pass2)) {
        ...
        #   CHECKING ERRORS AND CONTINUE SAVING USER'S ACCOUNT OR ELSSE RETURN AN ERROR MESSAGE TO THE BROWSER
        if ($errorMessages == "") {
            if ($newUser->saveUser()) {
				$act_code = User::getActivationCode($email);
				$act_link = "http://www.kgl.com/register.php?activate&email=$email&code=$act_code";
				$message = "
...
";
				$mailer = new Email();
				$mailer->sendMail($email, "KGL Book Store - Activation", $message);
	...
	//	HANDLES ACCOUNT ACTIVATION PROCESSES
