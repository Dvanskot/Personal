...
class User {
	...
	// Constructor creates a new user account or load data if already exist
	public function __construct($username = "new") {
		...
		if ($username != "new") {
			...
			### If username does not exists result in a fatal error and display the error
			if (!$result) {
				if (!headers_sent())
	...
	//static functions that checks the existance of data in the database
	...
	public static function login($username_email, $password) {
		...
		if ($result->num_rows > 0) {
			...
			if (!$myUser->validate())
				return "Internal System error, User account is invalid, please contact administrator";
			$_SESSION['user'] = $user;
	...
	public function saveUser() {
		if (!User::isExistUsername($this->username)) {
			...
			if ($this->username == "admin@smartreaders.com") {
				//Reserve "admin@smartreaders.com" as super admin
				$this->is_admin = 1;
				$this->is_active = 1;	
			}
			$this->validation = md5($this->username . $this->is_admin . $this->date_created->getDateBasic());
